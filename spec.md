...
- Evidence (chuẩn A và/hoặc B — log đầy đủ trong repo):
  - Số liệu mining / kết quả khảo sát — KHAI BÁO TRUNG THỰC TẠI CP4:
    ...
    - Mining chuẩn B-mầm (số do discord-pack cung cấp, tái kiểm được bằng script khi có file `k4_messages.csv`): 1092 tin (779 người, 313 bot), mentions_bot 307; channel_10 654 tin. Phương pháp đếm: script đếm `is_bot/mentions_bot/channel` + đọc 30–50 mẫu phân loại (logistics lặp / hỏi tech / hỏi code). File CSV hiện KHÔNG có trên máy (đã tìm toàn workspace) — số trên là số pack báo, chưa tự đếm lại; KHÔNG commit file này khi có. Đây là mầm loại pain (hỏi lặp, thiếu căn cứ), CHƯA phải evidence build-phase.
  - Quote nguyên văn build-phase (chuẩn ≥5): CHƯA ĐẠT (0/5 tại CP4; `validation/user_testing_log.md` chưa hoàn thiện (toàn bảng còn trống), chưa chạy outsider trial). Mầm minh hoạ loại pain (KHÔNG tính là evidence, mã msg chưa đối chiếu lại vì thiếu file CSV): M84888, M20982, M24912 + 2 quan sát trực tiếp phòng Discord team mình (ghi sau CP4). CẤM đoán "tin này của ai", cấm dán nguyên file.
...
|---|---|---|---|---|---|
| E1 (CHỌN) | Team coding companion trong Discord (check-tech + explain + propose-diff trên branch) | Team build-phase của chính nhóm (3–4 người, gặp trực tiếp được) — tần suất CHƯA ĐO (0 buổi quan sát có số liệu tại CP4) | 15–30' tranh luận/lần (ước lượng từ pain team mình, chưa survey) + quyết sai tech (làm lại 1–3h) / push nhầm main | Có — 1 lệnh Discord + 1 repo snapshot + 1 AI call |
...
- Ứng viên CHỌN + vì sao: E1 được chọn vì (a) tần suất pain cao nhất trong build phase theo quan sát định tính team mình (chưa có số liệu định lượng tại CP4 — bù sau CP4 bằng 3 buổi quan sát có đếm), (b) nhóm gặp được user thật (chính team build-phase), (c) demo được 1 quyết định AI trong 5'. E2/E3 để dành cho track B.
...
- Mức prototype nhắm tới: [ ] Sketch [x] Mock [ ] Working — Mock: flow Discord bấm được, demo CP3 bằng CLI mirror `app.py` (không cần token); data giả + repo mẫu, AI thật ở lõi (check-tech/explain/propose-diff). Phần mock: gateway Discord `bot.py` cần `DISCORD_TOKEN` mới chạy live; git push thật không có — chỉ tạo file `.patch` preview, không push.
...
|---|---|
| G1 — Làm rõ làm được gì | Mỗi slash command trong `bot.py` có description liệt kê đúng 4 lệnh + ví dụ; lệnh `/help` riêng chưa code (ghi nhận thiếu, không claims có) |
...
- Tự khai báo thiếu sót tại CP4 (không giấu): (1) run1 trong `results_run1.json` là STUB `simulate()` — 100% (24/24) không có giá trị đo; trace AI thật lẻ (`mocked=false` trong `codebase/outputs/`, 17/9) đã có nhưng full-set real eval 24 case vẫn pending; (2) golden set mới 1/24 case paraphrase từ chat thật (CT-06), còn thiếu ≥9 case real-chat (target ≥10) — bù sau CP4; (3) chiều Usefulness-concise (1–5) chưa chấm độc lập (0/5 output, target 2 người chấm 5 output — form chấm đã có trong `validation/grading_sheet.md`); (4) `course_kb.json` còn 2 announcement FAKE (ANN-001/002) — thay bằng id chính thức trước khi đo thật.
...
| Sau CP4 (dọn template) | Xóa placeholder `___`, sửa ref `bot.py:help` không tồn tại → mô tả đúng slash descriptions, làm rõ số pack là số báo chưa tự đếm (thiếu CSV), cập nhật trạng thái trace thật lẻ đã có | Còn thời gian trước demo; mọi sửa đều là khai báo chính xác hơn, không thêm claims mới, không sửa Bar |
